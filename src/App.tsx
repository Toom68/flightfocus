...
import { Plane, RotateCcw, Play, X } from 'lucide-react';
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
      <motion.div
        ...
        className="w-full max-w-sm bg-cabin-panel border border-white/[0.08] rounded-2xl p-6 shadow-panel"
      >
        <div className="flex items-center gap-3 mb-4">
          <div className="w-10 h-10 rounded-lg bg-cabin-accent/15 flex items-center justify-center">
            <Plane className="w-5 h-5 text-cabin-accent" />
          </div>
          <div>
            <h2 className="text-sm font-semibold text-white">Resume flight?</h2>
            <p className="text-xs text-gray-400">{info.fromCity} → {info.toCity}</p>
          </div>
        </div>
        ...
        <div className="space-y-2">
          <button
            onClick={onContinue}
            className="w-full py-3 bg-gradient-to-r from-cabin-accent to-blue-500 hover:shadow-glow text-white font-medium rounded-xl transition-all flex items-center justify-center gap-2"
          >
          ...
          <button
            onClick={onRestart}
            className="w-full py-3 bg-cabin-dim/60 hover:bg-cabin-dim text-gray-300 font-medium rounded-xl transition-all flex items-center justify-center gap-2"
          >
